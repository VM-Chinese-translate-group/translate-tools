...
class ParatranzUploader:
    ...
    def __init__(self, root: Tk) -> None:
        ...
        if len(self.token) != 32:
            messagebox.showwarning("警告", "当前未设置token，无法使用程序功能。请打开config.json输入您的token。")
    ...
    def create_widgets(self) -> None:
        ...
        entries = {"项目ID：": (0, self.create_entry), 
                   "文件ID：": (1, self.create_entry),
                   "译文路径：": (2, self.create_path_entry)}
        for label, (row, method) in entries.items():
            Label(self.root, text=label, font=("黑体", 12)).grid(row=row, column=0, padx=10, pady=10, sticky="w")
            method(row)
        
        Button(self.root, text="上传译文", style="Accent.TButton", command=self.run_upload_translation, width=20).grid(row=3, column=0, columnspan=4, pady=20)
        Checkbutton(self.root, text="暗色模式", style="Switch.TCheckbutton", command=sv_ttk.toggle_theme).grid(row=3, column=2, pady=5, padx=30, sticky='w')
        Button(self.root, text="下载译文", command=self.run_download_translation, width=20).grid(row=3, column=0, padx=30, pady=5)
        ...
        self.version_label = Label(self.root, text=f"当前版本：{self.DEFAULT_VERSION}", font=("黑体", 12))
        self.version_label.grid(row=5, column=0, columnspan=3, padx=10, pady=10, sticky="w")
        
        self.update_label = Label(self.root, text="", font=("黑体", 12))
        self.update_label.grid(row=6, column=0, columnspan=3, padx=10, pady=10, sticky="w")
    ...
    def create_path_entry(self, row: int) -> None:
        ...
        Button(self.root, text="...", command=self.select_file_path).grid(row=row, column=2, padx=10, pady=10)

    def select_file_path(self) -> None:
        file = filedialog.askopenfile(filetypes=(('JSON', '*.json'), ('全部文件', '*.*')))
        if file:
    ...
    async def put_translation(self, proj_id, str_id, body):
        if await self.send_data(f"https://paratranz.cn/api/projects/{proj_id}/strings/{str_id}", body) != 200:
...
def main() -> None:
    ...
    if py_version < (3, 9):
        messagebox.showerror("无法使用", 
                             f"当前Python版本过低，无法使用。当前版本为{py_version.major}.{py_version.minor}，请使用Python 3.9+（不支持Windows7）")
        sys.exit(0)
